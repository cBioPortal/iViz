...
(function(Vue, dc, iViz, $) {
  Vue.component('topChartMainTemplate', {
    template: ' <abstract-chart :data.sync="group.data" :type.sync="group.type"         :mappedpatients.sync="patientsync"' +
    ' :mappedsamples.sync="samplesync" :filters.sync="group.filters"  :attributes="group.attributes" v-for="group in groups"></abstract-chart>',
    props: [
    ...
    events: {
      ...
      'update-all-filters': function(updateType) {
        ...
        _.each(this.groups, function(group) {
          var _selectedCases = iViz.sync.selectByFilters(group.filters,
        ...
        var resultSelectedSamples = _.intersection(mappedSelectedSamples,
          _selectedSamplesByFiltersOnly);
        var resultSelectedPatients = iViz.util.idMapping(this.samplemap, resultSelectedSamples);
        this.patientsync = iViz.util.idMapping(this.samplemap, _selectedSamplesByFiltersOnly)
        this.samplesync = iViz.util.idMapping(this.patientmap, _selectedPatientsByFiltersOnly)
